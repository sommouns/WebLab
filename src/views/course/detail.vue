<template lang="html">
  <div class="course_detail animated fadeIn" v-if="loadFinish"> 
    <el-card class="box-card" style="position:relative">
      ...
      <div class="course_description">
        ...
        <div class="num_of_learnt" style="position:absolute;bottom:1rem;right:3rem;">
          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="实验列表" name="first">
              <div v-for="item in courseInfo.courseTempletes" :key="item.id">
                <div class="course_item clearfix" v-if="item.isexped !== false">
...
export default {
  ...
  methods: {
    async toLab(item) {
      ...
      } else if (courseTemplete.type === 2 || courseTemplete.type === 3) {
        this.$router.push(`/questions/${this.courseId}|${courseTemplete.id}`);
      }
    },
    next() {
  ...
  async created() {
    setTimeout(async () => {
      ...
      var token = JSON.parse(localStorage.getItem("token")).v;
...
.course_detail .course_description .content {
  ...
  -webkit-line-clamp: 6;  //这里是在第二行有省略号
  overflow: hidden;
}
